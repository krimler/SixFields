# Build spec: an opinionated Cluster API assembly

Working name: `capi-distro` (rename freely). This document is written to be handed to a
coding agent (Claude Code or similar). It is split into three parts:

- **Part A — CLAUDE.md**: always-on guardrails. Put this in the repo root as `CLAUDE.md`
  so the agent reads it on every session.
- **Part B — PLAN.md**: phased plan with acceptance criteria and tests per phase.
- **Part C — First prompt**: the message that kicks off the work.

Defaults are chosen so the agent can start immediately. Every default is listed in
"Open decisions" at the end; override any of them before starting.
...
## Part C — First prompt to the agent

```
Read CLAUDE.md and PLAN.md fully before doing anything.

Start Phase 0. Before writing any code:
1. Fill versions.env with current stable versions of everything listed, and tell me what
   you picked so I can confirm.
2. Bring up the dev loop (make dev-up) and write docs/api-snapshot.md from the pinned
   sources — not from memory. Show me the condition-type table you found.
3. Record the quickstart fixtures.

Then continue into Phase 1 without waiting for me. Use the bold defaults, record
anything you had to decide in DECISIONS.md, put anything on the must-ask list in
QUESTIONS.md with your recommendation, and keep STATUS.md current so I can pick up
from it whenever I'm back. I will be away most of the time; don't block on me.

Work test-first in small vertical slices, one failing test at a time, tree always green,
commit per slice. When every acceptance checklist is green, do the no-slop final pass as
its own commit before you call the phase done.

Rules that override everything else: pure core packages, recorded fixtures, one negative
test per denied field, no new CRDs, DECISIONS.md for anything not covered, the must-ask
list is the only thing that stops work. If you find that something in PLAN.md is wrong
about the pinned CAPI release, say so in STATUS.md, propose the fix, and take the most
reversible path in the meantime.
```
