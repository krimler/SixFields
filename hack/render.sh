...
export WORKLOAD_NODE_IMAGE WORKLOAD_K8S_VERSION K0S_VERSION K0SMOTRON_K0S_VERSION K0SMOTRON_K0S_VERSION
mkdir -p bin/render
for overlay in assembly/clusterclass/overlays/*/; do
  name=$(basename "$overlay")
  kustomize build "$overlay" \
    | envsubst '$WORKLOAD_NODE_IMAGE $WORKLOAD_K8S_VERSION $K0S_VERSION $K0SMOTRON_K0S_VERSION' > "bin/render/${name}.yaml"
